...
public class Stribog {
    ...
    public int[] getHash(String message) {
        return getHashX(message.chars().toArray());
    }

    protected int[] getHashX(int[] message) {
        int[] h = new int[64];
    ...
    protected int[] gN(int[] Nn, int[] h, int[] m) {
        int[] K = SPL(xorBytes(h, Nn));
        int[] t = E(K, m);
        int[] tmp = xorBytes(xorBytes(h, t), m);
        return tmp;
    }

    protected int[] SPL(int[] state) {
        return L(P(S(state)));
    }
    ...
    protected int[] E(int[] K, int[] m) {
        int[] result = xorBytes(K, m);
        int[] Ki = new int[64];
        System.arraycopy(K, 0, Ki, 0, 64);
        for (int i = 0; i < 12; i++) {
            result = SPL(result);
            Ki = keySchedule(Ki, i);
            result = xorBytes(result, Ki);
        }
        return result;
    }

    private int[] keySchedule(int[] ki, int i) {
            return SPL(xorBytes(ki, C[i]));
    }
